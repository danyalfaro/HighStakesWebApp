...
exports.requestOptions = function (lat, lon, days = 3, units = "m") {
  ...
  options[
    "uri"
  ] = `https://cors-anywhere.herokuapp.com/${apiUtil.HOST}/v1/geocode/${lat}/${lon}/forecast/daily/${d}day.json`;
  options.qs["units"] = u;
...
exports.handleResponse = function (res) {
  if (res && res.hasOwnProperty("forecasts")) {
    ...
    forecasts.forEach((daily) => {
      console.log(
        `daily-forecast: day ${daily.num} - High of ${daily["max_temp"]}, Low of ${daily["min_temp"]}`
      );
